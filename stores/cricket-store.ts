...
import { computeSplitAmounts } from '@/app/t/[slug]/lib/utils';
...
interface CricketState {
  ...
  // Actions — all scoped by teamId
  loadAll: (teamId: string, userId: string) => Promise<void>;
  reset: () => void;

  // Players
  addPlayer: (userId: string, data: { name: string; jersey_number: number | null; phone: string | null; player_role: string | null; batting_style: string | null; bowling_style: string | null; cricclub_id: string | null; shirt_size: string | null; email: string | null; designation: string | null; photo_url?: string | null }) => void;
  ...
  // Expenses
  addExpense: (userId: string, seasonId: string, data: { category: string; description: string; amount: number; expense_date: string }, createdBy?: string) => void;
  ...
  // Settlements
  addSettlement: (userId: string, seasonId: string, data: { from_player: string; to_player: string; amount: number; settled_date: string }) => void;
...
const EMPTY_STATE = {
  ...
  seasons: [] as CricketSeason[],
  expenses: [] as CricketExpense[],
  splits: [] as CricketExpenseSplit[],
  settlements: [] as CricketSettlement[],
  fees: [] as CricketSeasonFee[],
  sponsorships: [] as CricketSponsorship[],
  gallery: [] as GalleryPost[],
  galleryTags: [] as GalleryTag[],
  galleryComments: [] as GalleryComment[],
  galleryLikes: [] as GalleryLike[],
  commentReactions: [] as CommentReaction[],
  notifications: [] as GalleryNotification[],
};
...
export const useCricketStore = create<CricketState>((set, get) => ({
  ...
  reset: () => set({ ...EMPTY_STATE, selectedSeasonId: null, teamId: null, loading: true }),

  loadAll: async (teamId: string, userId: string) => {
    set({ loading: true, teamId });

    const supabase = getSupabaseClient();
    if (!supabase) { set({ loading: false }); return; }

    // Load ALL team data scoped by team_id
    const [playersRes, seasonsRes, expensesRes, splitsRes, settlementsRes, feesRes, sponsorsRes, galleryRes, galleryTagsRes, galleryCommentsRes, galleryLikesRes, commentReactionsRes, notificationsRes] = await Promise.all([
      supabase.from('cricket_players').select('*').eq('team_id', teamId).order('created_at'),
      supabase.from('cricket_seasons').select('*').eq('team_id', teamId).order('year', { ascending: false }),
      supabase.from('cricket_expenses').select('*').eq('team_id', teamId).order('expense_date', { ascending: false }),
      supabase.from('cricket_expense_splits').select('*'),
      supabase.from('cricket_settlements').select('*').eq('team_id', teamId).order('settled_date', { ascending: false }),
      supabase.from('cricket_season_fees').select('*').order('created_at'),
      supabase.from('cricket_sponsorships').select('*').order('created_at'),
      supabase.from('cricket_gallery').select('*').eq('team_id', teamId).order('created_at', { ascending: false }),
  ...
  addPlayer: (userId, data) => {
    ...
    const newPlayer: CricketPlayer = {
      id: localId, team_id: teamId, user_id: null, ...data,
  ...
  addSeason: (userId, data) => {
    ...
    const newSeason: CricketSeason = {
      id: localId, team_id: teamId, user_id: userId, ...data, season_type: data.season_type as CricketSeason['season_type'],
    ...
    if (isCloudMode()) {
      ...
      supabase?.from('cricket_seasons')
        .insert({ team_id: teamId, user_id: userId, name: data.name, year: data.year, season_type: data.season_type })
        .select().single()
  ...
  addExpense: (userId, seasonId, data, createdBy) => {
    ...
    const newExpense: CricketExpense = {
      id: expenseId, team_id: teamId, user_id: userId, season_id: seasonId,
    ...
    if (isCloudMode()) {
      ...
      supabase.from('cricket_expenses')
        .insert({
          team_id: teamId, user_id: userId, season_id: seasonId,
  ...
  deleteExpense: (id, deletedBy) => {
    const now = new Date().toISOString();
    set({ expenses: get().expenses.map((e) => e.id === id ? { ...e, deleted_at: now, deleted_by: deletedBy ?? null } : e) });
    if (isCloudMode()) {
      const supabase = getSupabaseClient();
  ...
  restoreExpense: (id) => {
    set({ expenses: get().expenses.map((e) => e.id === id ? { ...e, deleted_at: null, deleted_by: null } : e) });
    if (isCloudMode()) {
      const supabase = getSupabaseClient();
  ...
  addSettlement: (userId, seasonId, data) => {
    ...
    const newSettlement: CricketSettlement = {
      id: localId, team_id: teamId, user_id: userId, season_id: seasonId, ...data, created_at: now,
    };
    ...
    if (isCloudMode()) {
      ...
      supabase?.from('cricket_settlements')
        .insert({ team_id: teamId, user_id: userId, season_id: seasonId, ...data })
        .select().single()
  ...
  addGalleryPost: (userId, seasonId, photoUrls, caption, postedBy, tagPlayerIds) => {
    ...
    const newPost: GalleryPost = {
      id: localId, team_id: teamId, season_id: seasonId, user_id: userId,
      photo_url: photoUrls[0] ?? null, photo_urls: photoUrls.length > 0 ? photoUrls : null,
      caption, posted_by: postedBy, deleted_at: null, created_at: now,
    };
    ...
    if (isCloudMode()) {
      ...
      supabase.from('cricket_gallery')
        .insert({ team_id: teamId, user_id: userId, season_id: seasonId, photo_url: photoUrls[0] ?? null, photo_urls: photoUrls.length > 0 ? photoUrls : null, caption, posted_by: postedBy })
        .select().single()
