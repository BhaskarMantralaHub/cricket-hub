...
function ViewTabs({ active, onChange, playerCount, expenseCount }: {
  active: View; onChange: (v: View) => void; playerCount: number; expenseCount: number;
}) {
          ...
          >
            ...
            {badge > 0 && (
              <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-full min-w-[18px] text-center leading-none ${
                isActive ? 'bg-white/25 text-white' : 'bg-[var(--hover-bg)] text-[var(--dim)]'
              }`}>
                {badge}
              </span>
            )}
...
export default function TeamDashboard() {
  ...
  const { team, isTeamAdmin } = useTeamContext();
  const { loading, selectedSeasonId, setShowExpenseForm, players, expenses, fees, sponsorships } = useCricketStore();
  ...
  // Listen for notification click → switch to gallery
  useEffect(() => {
  ...
  useEffect(() => {
    if (team) document.title = team.name;
  }, [team]);
  ...
  return (
    <div className="relative min-h-screen w-full px-3 py-5 sm:px-4 lg:px-8 overflow-hidden">
      ...
      {/* Header — greeting + season selector */}
      <div className="mb-5 flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[13px] text-[var(--muted)] mt-0.5">{team?.name}</p>
        </div>
        <SeasonSelector />
      </div>
      ...
      {!selectedSeasonId ? (
        ...
        <>
          ...
          {isTeamAdmin && activeView === 'expenses' && (
