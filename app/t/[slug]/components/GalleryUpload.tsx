...
function MentionDropdown({ query, players, onSelect, onSelectAll, position }: {
  ...
  const filtered = players.filter((p) => p.is_active && p.name.toLowerCase().includes(query.toLowerCase())).sort((a, b) => a.name.localeCompare(b.name)).slice(0, 6);
    ...
    <div className="absolute z-10 w-[220px] rounded-xl overflow-hidden shadow-xl"
      ...
      {showAll && (
        <>
          <button onMouseDown={(e) => { e.preventDefault(); onSelectAll(); }}
            className="w-full flex items-center gap-2 px-3 py-2 text-[13px] text-left cursor-pointer hover:bg-[var(--hover-bg)] transition-colors font-semibold" style={{ color: 'var(--orange)' }}>
      ...
      {filtered.map((p) => (
        <button key={p.id} onMouseDown={(e) => { e.preventDefault(); onSelect(p); }}
          className="w-full flex items-center gap-2 px-3 py-2 text-[13px] text-left cursor-pointer hover:bg-[var(--hover-bg)] transition-colors" style={{ color: 'var(--text)' }}>
...
export default function GalleryUpload({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const reset = () => { previews.forEach((url) => URL.revokeObjectURL(url)); setFiles([]); setPreviews([]); setCaption(''); setUploading(false); setMentionQuery(null); };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((prev) => [...prev, ...toAdd]); setPreviews((prev) => [...prev, ...newPreviews]);
  ...
  const handleCaptionChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (atIndex >= 0) {
      ...
      if (!afterAt.includes('\n') && afterAt.length <= 30) {
        setMentionQuery(afterAt); setMentionStart(atIndex);
        setMentionPos({ top: (captionRef.current?.offsetHeight ?? 40) + 4, left: 0 }); return;
      }
    }
  ...
  const insertMention = (name: string) => {
    ...
    setCaption(`${before}@${name} ${after}`); setMentionQuery(null);
    setTimeout(() => { if (captionRef.current) { const pos = mentionStart + name.length + 2; captionRef.current.focus(); captionRef.current.setSelectionRange(pos, pos); } }, 0);
  };
  ...
  const handlePost = async () => {
    ...
    const postId = crypto.randomUUID(); const photoUrls: string[] = [];
    for (let i = 0; i < files.length; i++) {
      const compressed = await compressGalleryImage(files[i]);
      const path = `${selectedSeasonId}/${postId}_${i}.jpg`;
      const { error } = await supabase.storage.from('gallery-photos').upload(path, compressed, { contentType: 'image/jpeg' });
      if (error) { console.error('[gallery] upload failed:', error); setUploading(false); return; }
  ...
  useEffect(() => {
    if (!open) return;
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape' && mentionQuery !== null) { e.stopPropagation(); setMentionQuery(null); } };
  ...
  return (
    <Drawer.Root open={open} onOpenChange={(isOpen) => { if (!isOpen) handleClose(); }} direction="bottom">
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 sm:max-w-md sm:mx-auto rounded-t-2xl outline-none"
          style={{ background: 'var(--card)', border: '1px solid var(--border)', borderBottom: 'none' }} aria-describedby={undefined}>
          ...
          <div className="flex items-center justify-between px-5 py-3">
            <button onClick={handleClose} className="text-[14px] font-medium cursor-pointer min-w-[60px] text-left" style={{ color: 'var(--muted)' }}>Cancel</button>
            <Drawer.Title className="text-[16px] font-bold text-[var(--text)]">New Post</Drawer.Title>
            <button onClick={handlePost} disabled={files.length === 0 || uploading}
              className="flex items-center gap-1.5 text-[14px] font-semibold cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed min-w-[60px] justify-end" style={{ color: 'var(--blue)' }}>
              <Send size={14} /> {uploading ? 'Posting...' : 'Share'}
            </button>
          </div>
          ...
          <div className="px-5 pb-6 pt-4 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            {previews.length > 0 ? (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[12px] font-semibold uppercase tracking-wider text-[var(--muted)]">Photos ({previews.length}/{MAX_PHOTOS})</span>
                  {previews.length < MAX_PHOTOS && (
                    <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1 text-[12px] font-semibold cursor-pointer" style={{ color: 'var(--blue)' }}>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {previews.map((url, i) => (
                    <div key={i} className="relative flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden">
                      ...
                      <button onClick={() => removeFile(i)} className="absolute top-1 right-1 p-1 rounded-full bg-black/60 text-white cursor-pointer hover:bg-black/80 transition-colors"><X size={12} /></button>
                      <span className="absolute bottom-1 left-1 text-[10px] font-bold text-white bg-black/50 rounded px-1">{i + 1}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <button onClick={() => fileRef.current?.click()}
                ...
                style={{ background: 'linear-gradient(135deg, rgba(217,119,6,0.06), rgba(239,68,68,0.04))', border: '2px dashed var(--border)' }}>
                <div className="w-16 h-16 rounded-full flex items-center justify-center"
                  style={{ background: 'linear-gradient(135deg, rgba(217,119,6,0.15), rgba(239,68,68,0.1))' }}>
            ...
            <div className="relative">
              <label className="text-[12px] font-semibold uppercase tracking-wider text-[var(--muted)] mb-1.5 block">Caption</label>
              <textarea ref={captionRef} value={caption} onChange={handleCaptionChange}
                placeholder="Great match today! Use @ to tag players" rows={2} maxLength={500}
                className="w-full rounded-xl px-3 py-2.5 text-[14px] resize-none"
                style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }} />
              {mentionQuery !== null && <MentionDropdown query={mentionQuery} players={activePlayers}
            ...
            {taggedPlayers.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                ...
                {taggedPlayers.map((p) => (
                  <span key={p.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium"
                    style={{ background: 'var(--hover-bg)', color: 'var(--blue)' }}>@{p.name}</span>
                ))}
              </div>
            )}
          </div>
