  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4">
      <div className="w-24 h-24 rounded-full flex items-center justify-center mb-6"
        style={{ background: 'linear-gradient(135deg, rgba(217,119,6,0.12), rgba(239,68,68,0.08))' }}>
      ...
      <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={onUpload}
        className="flex items-center gap-2 px-6 py-3 rounded-full text-white text-[14px] font-semibold cursor-pointer shadow-lg"
        style={{ background: 'linear-gradient(135deg, var(--orange), var(--red))', boxShadow: '0 4px 16px rgba(217,119,6,0.3)' }}>
        <Camera size={18} /> Post First Photo
      </motion.button>
    </div>
  );
}
...
export default function Gallery() {
  ...
  return (
    <div className="relative min-h-[50vh]">
          ...
          <div className="max-w-lg mx-auto mb-6">
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-5">
                <StatItem icon={<ImageIcon size={14} style={{ color: 'var(--orange)' }} />} value={seasonPosts.length} label={seasonPosts.length === 1 ? 'post' : 'posts'} />
                <StatItem icon={<Heart size={14} fill="var(--red)" style={{ color: 'var(--red)' }} />} value={totalLikes} label={totalLikes === 1 ? 'like' : 'likes'} />
                <StatItem icon={<MessageCircle size={14} style={{ color: 'var(--blue)' }} />} value={totalComments} label={totalComments === 1 ? 'comment' : 'comments'} />
              </div>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} onClick={() => setShowUpload(true)}
                className="hidden sm:flex items-center gap-1.5 px-5 py-2.5 rounded-full text-white text-[13px] font-semibold cursor-pointer"
                style={{ background: 'linear-gradient(135deg, var(--orange), var(--red))', boxShadow: '0 2px 12px rgba(217,119,6,0.25)' }}>
          ...
          <div ref={feedParent} className="space-y-5 max-w-lg mx-auto">
            {seasonPosts.map((post, index) => (
              <GalleryPostCard key={post.id} post={post}
                ...
                reactions={commentReactions} players={players} index={index} />
            ))}
          </div>
      ...
      {seasonPosts.length > 0 && (
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowUpload(true)}
