        ...
        {activePlayers.sort((a, b) => a.net_balance - b.net_balance).map((b) => {
          ...
          return (
            <div key={b.player_id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-[var(--hover-bg)] transition-colors">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg text-[13px] font-bold"
                style={{ backgroundColor: settled ? 'var(--dim)' : isOwed ? 'var(--green)' : 'var(--red)', color: 'white', opacity: settled ? 0.5 : 1 }}>
                {b.jersey_number ? `#${b.jersey_number}` : b.player_name.charAt(0)}
              </div>
              <div className="flex-1">
                <span className={`text-[14px] font-medium ${settled ? 'text-[var(--muted)]' : 'text-[var(--text)]'}`}>{b.player_name}</span>
              </div>
