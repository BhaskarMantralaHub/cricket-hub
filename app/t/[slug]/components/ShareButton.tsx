...
function buildTextReport(store: ReturnType<typeof useCricketStore.getState>, teamName: string) {
...
async function generatePdf(storeState: ReturnType<typeof useCricketStore.getState>, teamName: string) {
  ...
  text(teamName, M + 22, 17, { size: 22, bold: true, color: WHITE });
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text(`${teamName}  |  ${season.name}`, M, H - 8);
...
export default function ShareButton() {
  ...
  const teamName = team?.name ?? 'Cricket Team';

  const handleDownloadPdf = async () => {
    setGenerating(true);
    try {
      const doc = await generatePdf(useCricketStore.getState(), teamName);
      if (doc) {
        doc.save(`${teamName.replace(/\s+/g, '_')}_${season.name.replace(/\s+/g, '_')}.pdf`);
      }
    } finally {
      setGenerating(false);
    }
  ...
  const handleSharePdf = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${teamName}_${season.name}.pdf`, { type: 'application/pdf' });

      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: `${teamName} — ${season.name}` });
      } else {
        // Fallback: download
        doc.save(`${teamName.replace(/\s+/g, '_')}_${season.name.replace(/\s+/g, '_')}.pdf`);
      }
