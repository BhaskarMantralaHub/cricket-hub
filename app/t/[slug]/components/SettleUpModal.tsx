...
export default function SettleUpModal() {
  ...
  const resetAndClose = () => {
    setFromPlayer(''); setToPlayer(''); setAmount('');
  ...
  const handleSubmit = () => {
    ...
    addSettlement(user.id, selectedSeasonId, {
      from_player: fromPlayer, to_player: toPlayer,
      amount: parseFloat(amount), settled_date: date,
    });
    resetAndClose();
  };
      ...
      <div className="fixed inset-x-4 top-[20%] z-50 mx-auto max-w-md rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-2xl animate-slide-in">
        ...
        <div className="mb-4">
          ...
          <select value={toPlayer} onChange={(e) => setToPlayer(e.target.value)}
            className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-[14px] text-[var(--text)] outline-none">
            <option value="">Select player</option>
            {activePlayers.filter((p) => p.id !== fromPlayer).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <div className="mb-4 flex gap-3">
          <div className="flex-1">
            ...
            <input type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)}
              className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-[14px] text-[var(--text)] outline-none" placeholder="0.00" />
          </div>
          <div className="w-40">
        ...
        <button onClick={handleSubmit} disabled={!fromPlayer || !toPlayer || !amount || fromPlayer === toPlayer}
