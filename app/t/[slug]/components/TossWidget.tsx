...
function playCoinSound() {
  try {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      osc.type = 'sine'; osc.frequency.value = 3500 + (i % 2) * 800;
      ...
      osc.start(now + t); osc.stop(now + t + 0.02);
    }
    [1.85, 1.93].forEach((time, i) => {
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.connect(gain); gain.connect(ctx.destination);
      osc.type = 'triangle'; osc.frequency.value = 1200 - i * 400;
      gain.gain.setValueAtTime(0.1 / (i + 1), now + time);
      gain.gain.exponentialRampToValueAtTime(0.001, now + time + 0.06);
      osc.start(now + time); osc.stop(now + time + 0.08);
    });
...
export default function TossWidget() {
  ...
  const handleToss = useCallback(() => {
    ...
    setIsFlipping(true); setResult(null); setShowParticles(false);
    playCoinSound();
    const outcome = fairToss();
    setTimeout(() => {
      setResult(outcome); setIsFlipping(false); setShowParticles(true);
  ...
  return (
    <div className="min-h-[calc(100vh-52px)] flex flex-col items-center px-4 py-6 relative overflow-hidden">
      <div className="relative w-full max-w-lg mx-auto mb-6">
        <img src="/toss.png" alt="Cricket Toss"
      ...
      <div className="text-center mb-4">
        <h1 className="text-[12px] uppercase tracking-[3px] text-[var(--muted)] font-medium mb-1">🏏 ICC Cricket Standard</h1>
        <h2 className="text-[24px] lg:text-[32px] font-bold bg-gradient-to-r from-[var(--purple)] via-[var(--blue)] to-[var(--indigo)] bg-clip-text text-transparent">Coin Toss</h2>
      </div>
      <div className="relative mb-4 lg:mb-8" style={{ perspective: '800px' }}>
        {showParticles && <Particles result={result} />}
        <div ref={coinRef} onClick={handleToss} className="relative w-[140px] h-[140px] lg:w-[220px] lg:h-[220px] cursor-pointer"
          data-result={result}
          style={{ transformStyle: 'preserve-3d',
            animation: isFlipping ? 'coinFlip 2s ease-out forwards' : !result ? 'coinIdle 3s ease-in-out infinite' : undefined,
            transform: !isFlipping && result ? result === 'tails' ? 'rotateY(180deg)' : 'rotateY(0deg)' : undefined,
            transition: !isFlipping && result ? 'transform 0.3s ease' : undefined }}>
          <div className="absolute inset-0 rounded-full flex items-center justify-center"
            style={{ backfaceVisibility: 'hidden', background: 'radial-gradient(circle at 35% 35%, #e8c87a, #b8860b 50%, #8b6914 100%)',
              boxShadow: result === 'heads' && !isFlipping ? '0 0 40px rgba(201, 145, 90, 0.4), inset 0 2px 8px rgba(255,255,255,0.25)' : '0 8px 32px rgba(0,0,0,0.3), inset 0 2px 8px rgba(255,255,255,0.25)',
              border: '4px solid rgba(232,200,122,0.3)' }}>
            <div className="text-center select-none">
              <div className="text-[56px] lg:text-[88px] font-black leading-none" style={{ color: '#5c3a1e', textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>H</div>
          ...
          <div className="absolute inset-0 rounded-full flex items-center justify-center"
            style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)',
              ...
              boxShadow: result === 'tails' && !isFlipping ? '0 0 40px rgba(156, 163, 175, 0.4), inset 0 2px 8px rgba(255,255,255,0.3)' : '0 8px 32px rgba(0,0,0,0.3), inset 0 2px 8px rgba(255,255,255,0.3)',
              border: '4px solid rgba(255,255,255,0.15)' }}>
            <div className="text-center select-none">
              <div className="text-[56px] lg:text-[88px] font-black leading-none" style={{ color: '#374151', textShadow: '0 2px 4px rgba(0,0,0,0.15)' }}>T</div>
              <div className="text-[10px] lg:text-[11px] font-bold text-[#4b5563] tracking-[3px] uppercase -mt-1">Tails</div>
            </div>
          </div>
        </div>
        <div className="mx-auto mt-4 rounded-full bg-black/20 blur-md transition-all duration-500"
          style={{ width: isFlipping ? '60px' : '120px', height: '12px', opacity: isFlipping ? 0.3 : 0.6 }} />
      </div>
      <div className="text-center mb-4 lg:mb-8 h-[50px] lg:h-[80px] flex flex-col items-center justify-center">
        ...
        {isFlipping && <div className="text-[18px] text-[var(--muted)] animate-pulse">Flipping...</div>}
        {!result && !isFlipping && <div className="text-[16px] text-[var(--dim)]">Tap the coin to toss</div>}
      </div>
      <button onClick={handleToss} disabled={isFlipping}
        ...
        style={{ background: 'linear-gradient(135deg, var(--purple), var(--indigo))', color: '#fff', boxShadow: '0 8px 30px rgba(139, 92, 246, 0.3)' }}>
      ...
      {history.length > 0 && (
        <div className="mt-10 w-full max-w-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {history.map((h, i) => (
              <div key={i} className="w-9 h-9 rounded-full flex items-center justify-center text-[14px] font-bold transition-all"
                style={{ background: h === 'heads' ? 'linear-gradient(145deg, #ffd700, #b8860b)' : 'linear-gradient(145deg, #c0c0c0, #808080)',
                  color: h === 'heads' ? '#78350f' : '#374151', opacity: 1 - i * 0.04,
      ...
      <div className="mt-10 max-w-sm mx-auto">
        <button onClick={() => setShowDisclaimer(!showDisclaimer)}
          ...
          <span>ⓘ</span><span>Fair Play Notice</span>
        </button>
        {showDisclaimer && (
          <div className="mt-2 text-[12px] text-[var(--dim)] text-center leading-relaxed animate-[slideIn_0.15s] bg-[var(--surface)] rounded-xl p-4 border border-[var(--border)]">
            ...
            <p>Each flip uses your device&apos;s cryptographic random number generator. No pattern, no memory, no bias. Every toss is an independent 50/50 event.</p>
          </div>
        )}
      </div>
    </div>
