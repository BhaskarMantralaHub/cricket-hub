  ...
  return (
    <>
      <button ref={btnRef} onClick={handleOpen}
        className="relative p-2 rounded-xl hover:bg-[var(--hover-bg)] cursor-pointer transition-colors" title="Notifications">
      ...
      {open && createPortal(
        <>
          ...
          <div className="fixed z-[99] w-[300px] max-h-[400px] rounded-2xl overflow-hidden shadow-2xl"
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
              ...
              {notifications.length > 0 && (
                <button onClick={handleClear} className="text-[11px] font-semibold cursor-pointer" style={{ color: 'var(--red)' }}>Clear all</button>
              )}
            </div>
            <div className="overflow-y-auto max-h-[340px]">
              {notifications.length === 0 ? (
                <div className="py-10 text-center"><p className="text-[13px] text-[var(--muted)]">All caught up!</p></div>
              ) : (
                notifications.slice(0, 30).map((n) => (
                  <button key={n.id} onClick={() => handleNotificationClick(n.id, n.post_id)}
